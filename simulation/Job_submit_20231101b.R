...
function_name <- "Simulation_2.R"

folder <- "row_diff_signal_quantile_20231101b"

N_total_rep <- 100
n_task_simul <- 20

model <- "poisson_indep__zip_indep"
lambda_true_1 <- c(2, 1.7, 1.2)
lambda_true_2 <- c(1.3, 1.6, 1.3)
lambda_true_3 <- c(1.6, 1.7, 1.2)
omega <- 0.1
n_chains <- 1
...
settings <- tibble(
  ...
  mutate(
    sbatchR_txt = glue::glue(
      ...
      --constrain=AVX512 \\
      {function_name} \\
      {lambda_true_1}_{lambda_true_2}_{lambda_true_3} \\
      {omega} \\
      {model} \\
      {n_chains} \\
      {folder}"
