...
#' @export lambda_draws MCMC samples for Poisson rate parameters $\lambda_{ij}$
#' @export contin_table_long Converted contingency table IJx1
...
pvbayes <- function(contin_table, 
  ...
  suppressMessages(
    table_long <- 
      ...
      mutate(
        lambda = str_c("lambda[", as.character(c(1:nrow(table_E))), "]")
      )
  )
  ...
  if (is.null(stan_obj)){
    ...
    mod <- eval(parse(text=glue::glue(".pvBayes$stanmodels${model}")))%>% 
